...
public class NodesStatusTest {
  ...
  public void testParseEndpointStatusString(){
    ...
    assertEquals(nodesStatus.endpointStates.iterator().next().sourceNode, "127.0.0.1");
    
    List<EndpointState> endpointsList = nodesStatus.endpointStates.iterator().next().endpoints.stream().collect(Collectors.toList());
    
    assertEquals(endpointsList.get(0).dc, "datacenter1");
    assertEquals(endpointsList.get(0).rack, "rack1");
    assertEquals(endpointsList.get(0).status, "NORMAL");
    assertEquals(endpointsList.get(0).endpoint, "127.0.0.1");
    assertEquals(endpointsList.get(0).hostId, "f091f82b-ce2c-40ee-b30c-6e761e94e821");
    assertEquals(endpointsList.get(0).tokens, "13");
    assertTrue(endpointsList.get(0).severity.equals((double)0.0));
    assertEquals(endpointsList.get(0).releaseVersion, "3.0.8");
    assertEquals(endpointsList.get(1).dc, "datacenter2");
    assertEquals(endpointsList.get(1).rack, "rack2");
    assertEquals(endpointsList.get(2).dc, "datacenter3");
    assertEquals(endpointsList.get(2).rack, "rack3");
